...
public class FileController {
	...
	public ResponseEntity<byte[]> display(String filePath, String category) throws IOException {
		File file = new File(getDirectoryPath(category), filePath);
		...
		header.set(HttpHeaders.CONTENT_TYPE, fileService.getContentType(file));
	...
	public ResponseEntity<List<FileDto>> upload(List<MultipartFile> multipartFiles, String category) {
		...
		if (multipartFiles != null && !multipartFiles.isEmpty()) {
			for (MultipartFile multipartFile : multipartFiles) {
				FileDto fileDto = fileService.upload(multipartFile, getDirectoryPath(category), DateUtil.getDatePath());
				files.add(fileDto);
			}
		}
	...
	@GetMapping(value = "/{boardNum}/count", produces = MediaType.APPLICATION_JSON_VALUE)
	...
	private String getDirectoryPath(String category) {
		return Objects.equals("download", category) ? DOWNLOAD_DIRECTORY : FILES_DIRECTORY;
	}
